...
<body class="bg-gray-100 min-h-screen">
  <div class="min-h-screen py-8">
    <div class="container mx-auto px-4 max-w-md">
      <div class="bg-white rounded-lg shadow-lg p-6">
                      ...
                      <div class="flex justify-between items-center">
                        <div>
                          <div class="font-medium text-gray-800">
                            {{ $bundleBill->waterUsage->billingPeriod->period_name }}</div>
                          <div class="text-xs text-gray-600">{{ $bundleBill->waterUsage->total_usage_m3 }} m³</div>
                        </div>
                        <div class="text-right">
                          <div class="font-semibold text-blue-600">Rp {{ number_format($bundleBill->total_amount) }}
                          </div>
                          <div class="text-xs text-gray-500">
                            {{ $bundleBill->due_date ? $bundleBill->due_date->format('d/m/Y') : 'Belum ditentukan' }}
                          </div>
                        </div>
                      </div>
                    </div>
                      ...
                      <div class="flex justify-between items-center">
                        <div>
                          <div class="text-sm font-medium text-gray-800">
                            {{ $paidBill->waterUsage->billingPeriod->period_name }}</div>
                          <div class="text-xs text-green-600">✓ Dibayar
                            {{ $paidBill->payment_date ? $paidBill->payment_date->format('d/m/Y') : 'Tanggal tidak diketahui' }}
                          </div>
                        </div>
                        <div class="text-right">
                          <div class="text-sm font-semibold text-gray-800">Rp
                            {{ number_format($paidBill->total_amount) }}</div>
                        </div>
                      </div>
                    </div>
                  @endforeach
                    ...
                    <div class="bg-white rounded px-3 py-2 border border-green-200">
                      <div class="flex justify-between items-center">
                        <div>
                          ...
                          <div class="text-xs text-green-600">✓ {{ $paidBundle->bill_count }} tagihan - Dibayar
                            {{ $paidBundle->paid_at ? $paidBundle->paid_at->format('d/m/Y') : 'Tanggal tidak diketahui' }}
                          </div>
                        </div>
                        <div class="text-right">
                          <div class="text-sm font-semibold text-gray-800">Rp
                            {{ number_format($paidBundle->total_amount) }}</div>
                        </div>
                      </div>
                    </div>
                  @endforeach
            ...
            @if (isset($availableUnpaidBills) && $availableUnpaidBills->count() > 0)
              <div class="bg-yellow-50 border border-yellow-200 rounded-lg p-4">
                  ...
                  @foreach ($availableUnpaidBills as $unpaidBill)
                    <div class="bg-white rounded px-3 py-2 border border-yellow-200">
                      <div class="flex justify-between items-center">
                        <div>
                          <div class="text-sm font-medium text-gray-800">
                            {{ $unpaidBill->waterUsage->billingPeriod->period_name }}</div>
                          <div class="text-xs text-gray-600">{{ $unpaidBill->waterUsage->total_usage_m3 }} m³ • Jatuh
                            tempo:
                            {{ $unpaidBill->due_date ? $unpaidBill->due_date->format('d/m/Y') : 'Belum ditentukan' }}
                          </div>
                        </div>
                        <div class="text-right">
                          <div class="text-sm font-semibold text-gray-800">Rp
                            {{ number_format($unpaidBill->total_amount) }}</div>
                          <div
                            class="text-xs {{ $unpaidBill->status === 'overdue' ? 'text-red-600' : 'text-yellow-600' }}">
                ...
                <div class="mt-3 text-xs text-yellow-700">
                  <p>💡 Tip: Anda dapat membayar tagihan ini dengan kembali ke halaman tagihan dan memilih bundle baru
                    atau pembayaran individual.</p>
                </div>
              </div>
            @endif
                  ...
                  @foreach ($pendingIndividualBills as $pendingBill)
                    <div class="bg-white rounded px-3 py-2 border border-orange-200">
                      <div class="flex justify-between items-center">
                        <div>
                          <div class="text-sm font-medium text-gray-800">
                            {{ $pendingBill->waterUsage->billingPeriod->period_name }}</div>
                          <div class="text-xs text-orange-600">⏳ Menunggu pembayaran</div>
                        </div>
                        <div class="text-right">
                          <div class="text-sm font-semibold text-gray-800">Rp
                            {{ number_format($pendingBill->total_amount) }}</div>
                        </div>
                      </div>
                    </div>
                  @endforeach
                  ...
                  @foreach ($pendingBundles as $pendingBundle)
                    <div class="bg-white rounded px-3 py-2 border border-indigo-200">
                      <div class="flex justify-between items-center">
                        <div>
                          <div class="text-sm font-medium text-gray-800">Bundle Payment</div>
                          <div class="text-xs text-indigo-600">⏳ {{ $pendingBundle->bill_count }} tagihan - Menunggu
                            pembayaran</div>
                        </div>
                        <div class="text-right">
                          <div class="text-sm font-semibold text-gray-800">Rp
                            {{ number_format($pendingBundle->total_amount) }}</div>
                        </div>
                      </div>
                    </div>
                  @endforeach
          ...
          <div class="bg-gray-50 rounded-lg p-4 mb-6">
            ...
            <div class="space-y-2 text-sm">
              <div class="flex justify-between">
                <span class="text-gray-600">Pelanggan:</span>
                <span
                  class="font-medium">{{ isset($customer) ? $customer->name : $bill->waterUsage->customer->name }}</span>
              </div>
              <div class="flex justify-between">
                <span class="text-gray-600">Kode:</span>
                <span
                  class="font-medium">{{ isset($customer) ? $customer->customer_code : $bill->waterUsage->customer->customer_code }}</span>
              </div>
                  ...
                  <div class="mt-1 space-y-1">
                    @foreach ($bills as $bundleBill)
                      <div class="text-xs bg-white rounded px-2 py-1">
                        {{ $bundleBill->waterUsage->billingPeriod->period_name }} - Rp
                        {{ number_format($bundleBill->total_amount) }}
                      </div>
                    @endforeach
                  </div>
                </div>
              ...
              <div class="flex justify-between font-bold text-lg">
                ...
                <span class="text-blue-600">Rp
                  {{ number_format(isset($total_amount) ? $total_amount : (isset($bill) ? $bill->total_amount : 0), 0, ',', '.') }}</span>
              </div>
          ...
          @if ($hasPendingPayment)
            ...
            <div class="bg-yellow-50 border border-yellow-200 rounded-lg p-4 mb-6">
              <div class="flex items-center">
                ...
                <div>
                  <h4 class="text-sm font-medium text-yellow-800">Bundle Payment Menunggu</h4>
                  <p class="text-sm text-yellow-700 mt-1">
                    Bundle payment sedang diproses. Silakan selesaikan pembayaran atau cek status.
                  </p>
                </div>
              </div>
            </div>

            <!-- Continue Bundle Payment -->
            <button
              onclick="location.href='{{ isset($pendingPayment) && $pendingPayment->transaction_ref ? 'https://tripay.co.id/checkout/' . $pendingPayment->transaction_ref : '#' }}'"
            ...
            <!-- Check Bundle Payment Status Button -->
        ...
        @if (
            (isset($showingBundleForm) && $showingBundleForm && !$hasPendingPayment) ||
                (!isset($bills) && isset($bill) && $bill->status !== 'paid' && !$hasPendingPayment))
          ...
          <div class="space-y-4">
            <div>
              ...
              <p class="text-sm text-gray-600 mb-4">Pilih email untuk menerima notifikasi pembayaran dan struk
                elektronik</p>

              <div class="space-y-3">
                <div class="relative">
                  <input type="radio"
                    id="{{ isset($showingBundleForm) && $showingBundleForm ? 'email_choice_village_bundle' : 'email_choice_village' }}"
                    name="email_choice" value="village" class="sr-only" checked>
                  <label
                    for="{{ isset($showingBundleForm) && $showingBundleForm ? 'email_choice_village_bundle' : 'email_choice_village' }}"
                    ...
                    <div class="flex items-center flex-1">
                      ...
                      <div class="flex-1">
                        ...
                        <p class="text-xs text-green-600 mt-1">📧
                          {{ $village->email ?: 'admin@' . $village->slug . '.pamdes.id' }}</p>
                      </div>
                    </div>
                ...
                <div class="relative">
                  <input type="radio"
                    id="{{ isset($showingBundleForm) && $showingBundleForm ? 'email_choice_personal_bundle' : 'email_choice_personal' }}"
                    name="email_choice" value="personal" class="sr-only">
                  <label
                    for="{{ isset($showingBundleForm) && $showingBundleForm ? 'email_choice_personal_bundle' : 'email_choice_personal' }}"
                    class="flex items-center justify-between p-4 border-2 border-gray-200 rounded-lg cursor-pointer hover:border-blue-300 transition-colors email-choice-card">
                    <div class="flex items-center flex-1">
            ...
            <div>
              <label
                for="{{ isset($showingBundleForm) && $showingBundleForm ? 'customer_email_bundle' : 'customer_email' }}"
                class="block text-sm font-medium text-gray-700 mb-1">
                <span id="email_field_label">Email (Otomatis dari Desa)</span> <span class="text-red-500">*</span>
              </label>
              <input type="email"
                id="{{ isset($showingBundleForm) && $showingBundleForm ? 'customer_email_bundle' : 'customer_email' }}"
                name="customer_email"
          ...
          <div>
            <label
              for="{{ isset($showingBundleForm) && $showingBundleForm ? 'customer_phone_bundle' : 'customer_phone' }}"
              class="block text-sm font-medium text-gray-700 mb-1">
              Nomor Telepon (Opsional)
            </label>
            <input type="tel"
              id="{{ isset($showingBundleForm) && $showingBundleForm ? 'customer_phone_bundle' : 'customer_phone' }}"
              name="customer_phone"
              ...
              <div>
                @if (isset($showingBundleForm) && $showingBundleForm)
                  ...
                  <p class="text-sm text-blue-700 mt-1">
                    ...
                    Scan QR code dengan aplikasi e-wallet atau mobile banking Anda untuk membayar semua tagihan
                    sekaligus.
                  </p>
  ...
  <script>
      ...
      function initializeEmailSelection(formContext) {
        ...
        function updateEmailSelection(choice) {
          ...
          if (choice === 'village') {
            ...
            if (emailHelperText) emailHelperText.innerHTML =
              'Email desa digunakan secara otomatis. Tim PAMDes akan menginformasikan status pembayaran kepada Anda.';
            ...
            if (emailHelperText) emailHelperText.innerHTML =
              'Masukkan email pribadi yang aktif untuk menerima notifikasi pembayaran dan struk elektronik.';
      ...
      document.addEventListener('DOMContentLoaded', function() {
        ...
        if (bundleForm) {
          ...
          `;
            ...
            // Show loading toast
            const loadingAlertId = createAlert('info', 'Sedang membuat transaksi...', 0);
    ...
    @if (isset($showingBundleForm) && $showingBundleForm && $hasPendingPayment)
      function checkPaymentStatus() {
        ...
        button.innerHTML = `
          <div class="flex items-center justify-center">
            ...
            Mengecek Status Bundle...
          </div>
        `;
        button.disabled = true;

        // Show info alert
        const checkingAlertId = createAlert('info', 'Sedang mengecek status bundle payment...', 0);
        ...
        .then(response => response.json())
          .then(data => {
            ...
            if (data.success && data.status === 'paid') {
              ...
              createAlert('success', 'Bundle payment berhasil! Halaman akan dikembalikan...', 2000);
              setTimeout(() => {
              ...
              switch (statusText) {
                case 'pending':
                  statusMessage =
                    'Bundle payment masih dalam proses. Silakan selesaikan pembayaran atau coba lagi dalam beberapa menit.';
                  createAlert('warning', statusMessage);
                  break;
                case 'unpaid':
                  statusMessage = 'Bundle payment belum dilakukan. Silakan lanjutkan pembayaran.';
                  createAlert('error', statusMessage);
                  break;
                case 'expired':
                  statusMessage = 'Bundle payment telah kedaluwarsa. Silakan buat pembayaran baru.';
                  createAlert('error', statusMessage);
                  ...
                  break;
                case 'failed':
                  statusMessage = 'Bundle payment gagal. Silakan coba lagi.';
                  createAlert('error', statusMessage);
                  // Reload page after 3 seconds to show updated form
                  setTimeout(() => {
                    location.reload();
                  ...
                  break;
                default:
                  statusMessage = `Status bundle payment: ${statusText}`;
          ...
          .catch(error => {
            ...
            // Show error alert
            createAlert('error', 'Gagal mengecek status bundle payment. Silakan coba lagi.');
          });
      }
    @elseif (!isset($payment_type) || $payment_type !== 'bundle')
      @if (isset($bill) && $hasPendingPayment)
        function checkPaymentStatus() {
          const button = event.target;
          const originalText = button.innerHTML;
              ...
              if (data.success && data.status === 'paid') {
                ...
                setTimeout(() => {
                  window.location.href =
                    `{{ isset($bill) ? route('portal.bills', $bill->waterUsage->customer->customer_code) : (isset($customer) ? route('portal.bills', $customer->customer_code) : '#') }}`;
                }, 2000);
              } else {
                // Reset button
                button.innerHTML = originalText;
